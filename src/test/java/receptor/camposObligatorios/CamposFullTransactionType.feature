Feature: Ejecutar Endpoint para crear una transaccion aprobada y validando distintos datos de respuesta
  ...
  Background:
    * def bodyTrxCorr = read('classpath:helper/camposObligatorios/TransacctionTypeFull.json')
  ...
  Scenario Outline: peticion Post que obtiene el token de cognito y luego lo pasa al header de la peticion de aprobacion
    * def sleep = function(pause){ java.lang.Thread.sleep(pause)}
    ...
    Examples:
      ...
      #| RECARGA NEQUI           |
      #| PAGO FACTURA MANUAL     |
      #| PAGO FACT. CÓD BARRAS   | # bug "error": "Transaction type is not valid"
      #| PAGO FACT. TARJ EMPRES  |
      | PAGO TARJETA DE CRÉDITO | # bug "error": "Transaction type is not valid"
      #| PAGO CARTERA            |
      #| CONSULTA DE SALDO       |
      #| CONSULTAR CUPO          |
      #| CAMBIO DE CLAVE         |